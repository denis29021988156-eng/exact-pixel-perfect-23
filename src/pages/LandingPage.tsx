...
const FLOW_SOURCES = [
  { icon: Send, label: 'Telegram-бот для сотрудников', hint: 'фото + текст с места' },
  { icon: FileSpreadsheet, label: 'Excel-выгрузки', hint: 'от департаментов' },
  { icon: FileText, label: 'Письма и обращения', hint: 'свободным текстом' },
  { icon: Plus, label: 'Ручной ввод инцидента', hint: 'в один клик' },
];
...
const FLOW_LEVELS = [
  {
    ...
    text: 'Сотрудники вносят город в систему — четырьмя способами.',
    footnote: 'В перспективе — прямой канал для жителей.',
  },
  {
    kicker: 'Уровень 2',
    title: 'AI разбирает каждое сообщение',
    text: 'Из любого текста за 2 секунды достаёт: что случилось · где · насколько срочно · кто отвечает.',
    footnote: 'Сотрудник только подтверждает кнопкой — не переписывает.',
  },
  {
    kicker: 'Уровень 3',
    title: 'Система считает картину города',
    text: 'Индекс риска города · Прогноз срыва сроков · Бюджетные риски · Зоны напряжения на карте.',
    footnote: 'Всё автоматически, без планёрок и сводок.',
  },
];
...
const TEAM_ROLES = [
  ...
  {
    icon: Shield,
    name: 'Зам / Руководитель департамента',
    desc: 'Видит свою зону. Управляет командой. Подгружает данные.',
    screens: ['Сегодня', 'Карта', 'Инциденты', 'Программа', 'Поручения', 'Репутация', 'AI-шпаргалка', 'Источники данных'],
  },
  {
    icon: User,
    name: 'Сотрудник',
    desc: 'Закрывает инциденты и поручения по своему участку.',
    screens: ['Сегодня', 'Инциденты', 'Поручения'],
  },
  {
    icon: Settings,
    name: 'Администратор системы',
    desc: 'Настраивает платформу. Город не видит.',
    screens: ['Матрица SLA', 'Пользователи', 'Аудит-лог', 'Feature flags', 'Источники данных'],
  },
];
